...
// Esto es nuevo, se comentarea para que no se recalcule el valor de la cuota
{
            if EsVivienda then
             begin
...
// Esto es nuevo, se deja calculo de tabla cuota fija
             CrearTablaLiquidacion(vIDColocacion,vDesembolso,vValorCuota,
                        CalculoFecha(vFechaDesembolso,vPeriodoGracia+vDiasPago),vTasaEfectiva,vPuntosAdicionales,
                        vAmortizaCapital,vAmortizaInteres,vPlazo,vCuotaTipo,vInteresVencido, IBQuery);
...
// Esto es nuevo, se comentarea la actualizacion del valor de la cuota
{
            with IBVarios do
             begin
               sql.Clear;
...
//                     if (AbonoK <= vAbonosCapital) and (FieldByName('FECHA_A_PAGAR').AsDateTime <= vFechaInteres) then
   ...
   DmColocacion.dbGeneral.IBTransaction1.Commit;
   dmGeneral.IBTransaction1.Commit;
   MessageDlg('Operación Exitosa',mtCustom,[mbOK],0);
   CmdCerrar.SetFocus;
  Except
   DmColocacion.dbGeneral.IBTransaction1.Rollback;
