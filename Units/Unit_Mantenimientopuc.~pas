...
interface
 ...
 type
    ...
    procedure CrearCuentasFaltantes;
...
procedure TfrmMantenimientopuc.CrearCuentasFaltantes;
var
 IBQSQLpuc: TIBSQL;
 IBQSQLins: TIBSQL;
 IBQSQLbus: TIBSQL;
 CodigoMayorBuscar, CodigoMayorCrear : String;
 NivelCrear: Integer;
begin

        IBQSQLpuc := TIBSQL.Create(nil);
        IBQSQLins := TIBSQL.Create(nil);
        IBQSQLbus := TIBSQL.Create(nil);

        IBQSQLpuc.Database := dmGeneral.IBDatabase1;
        IBQSQLins.Database := dmGeneral.IBDatabase1;
        IBQSQLbus.Database := dmGeneral.IBDatabase1;

        IBQSQLpuc.Transaction := dmGeneral.IBTransaction1;
        IBQSQLins.Transaction := dmGeneral.IBTransaction1;
        IBQSQLbus.Transaction := dmGeneral.IBTransaction1;
        ...
        IBQSQLins.Close;
           IBQSQLins.sql.Clear;
           IBQSQLins.sql.Add('insert into "con$puc"');
           IBQSQLins.sql.Add('("con$puc"."CODIGO", "con$puc"."ID_AGENCIA",');
           IBQSQLins.sql.Add('"con$puc"."CLAVE", "con$puc"."NOMBRE",');
           IBQSQLins.sql.Add('"con$puc"."TIPO", "con$puc"."CODIGOMAYOR",');
           IBQSQLins.sql.Add('"con$puc"."MOVIMIENTO", "con$puc"."NIVEL",');
           IBQSQLins.sql.Add('"con$puc"."ESBANCO", "con$puc"."NATURALEZA",');
           IBQSQLins.sql.Add('"con$puc"."INFORME", "con$puc"."CENTROCOSTO",');
           IBQSQLins.sql.Add('"con$puc"."SALDOINICIAL","con$puc".ESCAPTACION,"con$puc".ESCOLOCACION)');
           IBQSQLins.sql.Add('values (');
           IBQSQLins.sql.Add(':"CODIGO", :"ID_AGENCIA", :"CLAVE", :"NOMBRE",');
           IBQSQLins.sql.Add(':"TIPO", :"CODIGOMAYOR", :"MOVIMIENTO", :"NIVEL",');
           IBQSQLins.sql.Add(':"ESBANCO", :"NATURALEZA", :"INFORME",');
           IBQSQLins.sql.Add(':"CENTROCOSTO", :"SALDOINICIAL",:"ESCAPTACION",:"ESCOLOCACION")');

           IBQSQLbus.SQL.Clear;
           IBQSQLbus.SQL.Add('SELECT * FROM "con$puc" p WHERE p.CODIGO = :CODIGO');

           IBQSQLpuc.close;
           IBQSQLpuc.SQL.Clear;
           IBQSQLpuc.SQL.Add('SELECT * FROM "con$puc" p ORDER BY p.CODIGO');
           IBQSQLpuc.ExecQuery;

        while not IBQSQLpuc.Eof do
        begin

           CodigoMayorBuscar := EvaluarCodigoMayor(IBQSQLpuc.FieldByName('CODIGO').AsString);
           IBQSQLbus.Close;
           IBQSQLbus.ParamByName('CODIGO').AsString := CodigoMayorBuscar;
           IBQSQLbus.ExecQuery;
           if (IBQSQLbus.RecordCount = 0) then
           begin

            CodigoMayorCrear := CodigoMayorBuscar;
            NivelCrear := EvaluarNivel(CodigoMayorCrear);

            IBQSQLins.Close;
            IBQSQLins.ParamByName('CODIGO').AsString         := CodigoMayorCrear;
            IBQSQLins.ParamByName('ID_AGENCIA').AsInteger    := agencia;
            IBQSQLins.ParamByName('CLAVE').AsString          := '00000';
            IBQSQLins.ParamByName('NOMBRE').AsString         := IBQSQLpuc.FieldByName('NOMBRE').AsString;
            IBQSQLins.ParamByName('TIPO').AsString           := '00';
            IBQSQLins.ParamByName('CODIGOMAYOR').AsString    := CodigoMayorCrear;
            IBQSQLins.ParamByName('MOVIMIENTO').AsInteger    := 0;
            IBQSQLins.ParamByName('NIVEL').AsInteger         := NivelCrear;
            IBQSQLins.ParamByName('ESBANCO').AsInteger       := 0;
            IBQSQLins.ParamByName('ESCAPTACION').AsInteger   := 0;
            IBQSQLins.ParamByName('ESCOLOCACION').AsInteger  := 0;
            IBQSQLins.ParamByName('NATURALEZA').AsString     := 'N';
            IBQSQLins.ParamByName('INFORME').AsString        := '00';
            IBQSQLins.ParamByName('CENTROCOSTO').AsString    := '00';
            IBQSQLins.ParamByName('SALDOINICIAL').AsCurrency := 0;
            IBQSQLins.ExecQuery;
           end;
           IBQSQLpuc.Next;
        end;
