...
begin
        ...
        begin
          ...
          SQL.Add('"col$solicitud"."INVERSION" = :INVERSION, ');
          SQL.Add('"col$solicitud"."CLASIFICACION" = :CLASIFICACION, ');
          ...
          ParamByName('VALOR_APROBADO').AsCurrency := JVvalorprestamo.Value;
          ParamByName('PAGO_INTERES').AsInteger := JVpagointeres.Value;
          ParamByName('PLAZO').AsInteger := JVplazo.Value;
          ParamByName('PLAZO_APROBADO').AsInteger := JVplazo.Value;
          ...
          //else
          ParamByName('ESTADO').AsInteger := 4;
          ...
          ParamByName('CLASIFICACION').AsInteger := 2;
          ParamByName('INVERSION').AsInteger := 2;
          ParamByName('TIPO_CUOTA').AsInteger := DBtipocuota.KeyValue;
...
//          end;
          ...
          begin
            ...
            begin
            {
              IBQuery1.Close;
              IBQuery1.SQL.Clear;
              IBQuery1.SQL.Add('SELECT SUM(CAPITAL) AS CAPITAL, SUM(INTERES) AS INTERES FROM GEN$DESCUENTOCARTERA WHERE ID_COLOCACION = :ID_COLOCACION AND APLICADO = 0');
              IBQuery1.ParamByName('ID_COLOCACION').AsString := FieldByName('ID_COLOCACION').AsString;
              IBQuery1.Open;
              Capital := IBQuery1.FieldByName('CAPITAL').AsCurrency;
              Interes := IBQuery1.FieldByName('INTERES').AsCurrency;
              }
              Saldo := FieldByName('saldo').AsCurrency;
              {
              FechaK := FieldByName('FECHA_CAPITAL').AsDate;
              FechaI := FieldByName('FECHA_INTERES').AsDate;
              if (Capital > 0) then
              begin
               Saldo := Saldo - Capital;
               FechaK := CalculoFecha(FechaK, 30);
               FechaI := CalculoFecha(FechaI, 30);
              end;
              }
              ...
              CDinfCrediticia.FieldValues['saldo'] := Saldo;
              ...
              CDinfCrediticia.FieldValues['fecha_capital'] := FieldByName('FECHA_CAPITAL').AsDate;
              CDinfCrediticia.FieldValues['fecha_interes'] := FieldByName('FECHA_INTERES').AsDate;
...
//        es_lineasocial := False;
        ...
        begin
        edMeses.Value := JVplazo.Value div 30;
        {busca_linea;
...
procedure TFrmInformacion.btnBuscarPersonaClick(Sender: TObject);
var
...
begin
                ...
                begin
                   DBidentificacion.KeyValue := frmBuscarPersona.id_identificacion;
                   JvDocumento.Text := frmBuscarPersona.id_persona;
                   jvDocumento.OnExit(self);
                end;
