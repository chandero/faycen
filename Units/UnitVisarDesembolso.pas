        ...
        begin
                ...
                begin
                     if FieldByName('ES_NOVISADO').AsInteger <> 1 then
                      ...
                      vNombre := FieldByName('PRIMER_APELLIDO').AsString  + ' ' + FieldByName('SEGUNDO_APELLIDO').AsString  + ' ' + FieldByName('NOMBRE').AsString ;
                      EdNombre.Caption := vNombre;
...
begin
            ...
            dmGeneral.IBTransaction1.StartTransaction;
                ...
                if (dmColocacion.IBDSagencias.Transaction.InTransaction) then dmColocacion.IBDSagencias.Transaction.Rollback;
                dmColocacion.IBDSagencias.Transaction.StartTransaction;

                if (dmSolicitud.IBSolicitud1.Transaction.InTransaction) then dmSolicitud.IBSolicitud1.Transaction.Rollback;
...
begin
        ...
        dmColocacion.IBDSagencias.Database := dmGeneral.IBDatabase1;
        ...
        dmColocacion.IBDSagencias.Transaction := dmGeneral.IBTransaction1;
...
procedure TfrmVisarDesembolso.btnChequeClick(Sender: TObject);
var
  Valor: Currency;
  Anyo: Word;
  Mes: Word;
...
// Leer Nota Contable
        ...
        IBAux.Close;
        IBAux.SQL.Clear;
        IBAux.SQL.Add('select * from "col$concol" a');
        IBAux.SQL.Add('left join "con$puc" p on a.CODIGO = p.CODIGO ');
        IBAux.SQL.Add('where');
        IBAux.SQL.Add('a.ID_AGENCIA = :ID_AGENCIA and ');
        IBAux.SQL.Add('a.ID_COLOCACION = :ID_COLOCACION');
        IBAux.ParamByName('ID_AGENCIA').AsInteger := Agencia;
        IBAux.ParamByName('ID_COLOCACION').AsString := vColocacion;
        try
          IBAux.Open;
          IBAux.First;
          while not IBAux.Eof do
          begin
              if (IBAux.FieldByName('ESBANCO').AsInteger = 1) then
              begin
                Valor := IBAux.FieldByName('CREDITO').AsCurrency;
                break;
              end;
              IBAux.Next;
          end;
          NumeroALetra.Numero := StrToInt(CurrToStr(Valor));
          ValorEnLetras := UpperCase(NumeroALetra.Letras);
          DecodeDate(Now, Anyo, Mes, Dia);
          Portador := EdNombre.Caption;
          IBAux.First;
          txtCheque.Variables.ByName['VALOR'].AsString := PadWithAsterisk(FormatFloat('#,##0.00',Valor),18);
