...
uses
  ...
  IBSQL, IBDatabase, Math, pr_Common, pr_TxClasses, ComCtrls, JvComponent, UnitDmGeneral,
  JvSimpleXml, IdBaseComponent, IdComponent, IdTCPConnection, IdTCPClient, sdXmlDocuments;

type
  TfrmConsignacionAportes = class(TForm)
    Panel1: TPanel;
    Panel2: TPanel;
...
procedure TfrmConsignacionAportes.Inicializar;
var AR:PTablaConNal;
begin
...
procedure TfrmConsignacionAportes.SGConsGetCellAlignment(
...
procedure TfrmConsignacionAportes.SGConsSelectCell(Sender: TObject; ACol,
...
procedure TfrmConsignacionAportes.FormShow(Sender: TObject);
begin
        Inicializar;
end;

procedure TfrmConsignacionAportes.AdicionarBanco(ARow:Integer);
...
procedure TfrmConsignacionAportes.FormCreate(Sender: TObject);
var
...
begin
  ...
  IBQTipoAhorro.Database := dmGeneral.IBDatabase1;
  ...
  IBQBancos.Transaction := IBTransaction1;
  IBQTipoAhorro.Transaction := IBTransaction1;
  IBSQL1.Transaction := IBTransaction2;
  IBQAgencia.Transaction := IBTransaction1;
  IBComprobante.Transaction := IBTransaction1;
  IBAuxiliar.Transaction := IBTransaction1;
  IBAuxiliar1.Transaction := IBTransaction1;
...
procedure TfrmConsignacionAportes.SGConsExitCell(Sender: TJvStringGrid;
...
procedure TfrmConsignacionAportes.DBLCBBancosExit(Sender: TObject);
var AR:PTablaConNal;
    Veces:Integer;
begin
...
procedure TfrmConsignacionAportes.FormKeyPress(Sender: TObject;
  var Key: Char);
begin
        EnterTabs(Key,Self);
end;

procedure TfrmConsignacionAportes.DBLCBBancosEnter(Sender: TObject);
begin
        (Sender as TDBLookupComboBox).DropDown;
end;

procedure TfrmConsignacionAportes.DBLCBTipoAhorroEnter(Sender: TObject);
begin
        (Sender as TDBLookupComboBox).DropDown;
end;

procedure TfrmConsignacionAportes.AdicionarTipoAhorro(ARow:Integer);
var NuevoDblcb:TDBLookupComboBox;
    R :TRect;
begin
        NuevoDblcb := TDBLookupComboBox.Create(Application);
...
procedure TfrmConsignacionAportes.AdicionarCredito(ARow:Integer);
...
procedure TfrmConsignacionAportes.cmdCerrarClick(Sender: TObject);
begin
        Close;
end;

procedure TfrmConsignacionAportes.chkCreditoExit(Sender: TObject);
var AR:PTablaConNal;
begin
        AR := Lista.Items[SGCons.Row - 1];
        if (Sender as TCheckBox).Checked then begin
           AR^.IdDest := 1;
           AR^.DeDest := 'SALIDA DE ' + AR^.DeTipo;
        ...
        begin
           AR^.IdDest := 0;
           AR^.DeDest := 'ENTRADA DE ' + AR^.DeTipo;
           AR^.Comis := AR^.ComisBanco;
           SGCons.Cells[7,SGCons.Row] := FormatCurr('$#,0.00',AR^.Comis);
        end;
        SGCons.Col := 6;
...
procedure TfrmConsignacionAportes.SGConsClick(Sender: TObject);
begin
        SGCons.SetFocus;
end;

procedure TfrmConsignacionAportes.DBLCBAgenciaExit(Sender: TObject);
var AR:PTablaConNal;
begin
        if (Sender as TDBLookupComboBox).Text = '' then Exit;
        AR := Lista.Items[(Sender as TDBLookupComboBox).Tag-1];
...
procedure TfrmConsignacionAportes.DBLCBAgenciaEnter(Sender: TObject);
begin
        (Sender as TDBLookupComboBox).DropDown;
end;

procedure TfrmConsignacionAportes.DBLCBTipoAhorroExit(Sender: TObject);
var AR:PTablaConNal;
begin
        if (Sender as TDBLookupComboBox).Text = '' then Exit;
        try
...
procedure TfrmConsignacionAportes.AdicionarAgencia(ARow: integer);
var NuevoDblcb:TDBLookupComboBox;
    R :TRect;
begin
        NuevoDblcb := TDBLookupComboBox.Create(Application);
...
procedure TfrmConsignacionAportes.SGConsGetEditText(Sender: TObject; ACol,
...
procedure TfrmConsignacionAportes.chkCreditoClick(Sender: TObject);
var AR:PTablaConNal;
begin
        AR := Lista.Items[SGCons.Row - 1];
        if (Sender as TCheckBox).Checked then begin
...
procedure TfrmConsignacionAportes.SGConsKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TfrmConsignacionAportes.FormCloseQuery(Sender: TObject;
...
procedure TfrmConsignacionAportes.cmdProcesarClick(Sender: TObject);
...
// Codigo Impuesto Asumido
             ...
             with IBComprobante do begin
                Close;
                SQL.Clear;
                SQL.Add('select * from "gen$compdescripcion" where ID_COMPROBANTE = :ID_COMPROBANTE');
                if (AR^.IdDest = 1) then
                  ParamByName('ID_COMPROBANTE').AsInteger := 3
                else
                  ParamByName('ID_COMPROBANTE').AsInteger := 2;
                try
                  Open;
...
{             if AR^.IdAgencia <> Agencia then
             ...
             if (AR^.IdDest = 1 ) Then
                 Consecutivo := ObtenerConsecutivo(IBSQL1,3)
             else
                 Consecutivo := ObtenerConsecutivo(IBSQL1,2);
             with IBComprobante do begin
                Close;
                SQL.Clear;
                ...
                ParamByName('ID_AGENCIA').AsInteger := Agencia;
                if AR^.IdDest = 1 then
                   ParamByName('TIPO_COMPROBANTE').AsString := '3'
                else
...
//
             ...
             if (AR^.IdDest = 1) then
                   ...
                   TipoNota := '2';
             IBComprobante.Transaction.Commit;
             CBNotas.Items.Add(Format('%s:%.8d',[TipoNota,Consecutivo]));
         end; // del if
        end; // del for
...
procedure TfrmConsignacionAportes.cmdReporteClick(Sender: TObject);
var i:Integer;
    Cadena: TStringList;
begin
        ...
        with IBAuxiliar do begin
          ...
          for i:=0 to CBNotas.Items.Count - 1 do begin
             ExtractStrings([':'], [], PChar(CBNotas.Items[i]),Cadena);
             SQL.Add('(("con$comprobante".TIPO_COMPROBANTE = '+ QuotedStr(Cadena[0]) + ') and ');
             SQL.Add('("con$comprobante".ID_COMPROBANTE = '+ Cadena[1] +') )');
...
procedure TfrmConsignacionAportes.reporteInitDetailBandDataSet(
...
procedure TfrmConsignacionAportes.AdicionarFecha(ARow: integer);
...
procedure TfrmConsignacionAportes.EdFechaExit(Sender: TObject);
var
 AR:PTablaConNal;
 ARow :Integer;
begin
...
procedure TfrmConsignacionAportes.statusBarDrawPanel(
...
procedure TfrmConsignacionAportes.IdTCPClient1WorkBegin(Sender: TObject;
...
procedure TfrmConsignacionAportes.IdTCPClient1Work(Sender: TObject;
...
procedure TfrmConsignacionAportes.IdTCPClient1WorkEnd(Sender: TObject;
  AWorkMode: TWorkMode);
begin
