...
type
  TfrmCalculoCuotadePrueba = class(TForm)
    ...
    Label38: TLabel;
    EdAportes: TJvCurrencyEdit;
    Label39: TLabel;
    EdDescuento: TJvCurrencyEdit;
...
var CuotasLiq:TCuotasLiq;
    ...
    vAportesAAbonar: Currency;
    vDescuentoAAbonar: Currency;
...
//     TipoCapital   := FieldByName('CAPITAL').AsString;
  ...
  vAportesAAbonar := EdAportes.Value;
  vDescuentoAAbonar := EdDescuento.Value;

  vEfectivo := EdEfectivo.Value;
  vNacional := EdNacional.Value;
  vLibranza := EdLibranza.Value;

  if (vEfectivo > 0) then
  begin
     vEfectivo := vEfectivo - vAportesAAbonar - vDescuentoAAbonar;
     vNacional := 0;
     vLibranza := 0;
  end
  else
  ...
  begin
     vEfectivo := 0;
     vNacional := vNacional - vAportesAAbonar - vDescuentoAAbonar;
     vLibranza := 0;
  end
  else
  begin
     vEfectivo := 0;
     vNacional := 0;
     vLibranza := vLibranza - vAportesAAbonar - vDescuentoAAbonar;
  end;
  ...
  if (TipoCuota = 1) and (CredPagoTotal or CredFallecido or CredIncapacitado) then
   LiquidarCuotasFijaPagoTotal(EdAgencia.KeyValue,EdNumeroColocacion.Text,StrToInt(EdCuotas.Text),EdFechaCorte.Date,
                     CuotasLiq,Clasificacion,Garantia,Categoria,Estado,Linea, ValorCuota,
 ...
 if vNacional > 0 then begin
  ...
  with IBSaldo do
   begin
     ...
     Close;
     SQL.Clear;
     SQL.Add('select CODIGO from "col$codigospucbasicos" where ID_CODIGOPUCBASICO = 63');
     ExecQuery;
     CodigoDescuento := FieldByName('CODIGO').AsString;
     ...
     DigitoCuenta := StrToInt(DigitoControl(1,FormatCurr('0000000',FieldByName('NUMERO_CUENTA').AsInteger)));
     Close;
     ...
     for I := 0 to CuotasLiq.Lista.Count - 1 do
      begin
        ...
        if AF^.CodigoPuc = CodigoCaja then begin
          ...
          else if vLibranza > 0 then Codigo := CodigoLibranza
          else if vEfectivo > 0 then Codigo := CodigoCaja
...
//          CuotasLiq.Lista.Pack;
     ...
     AR^.Tasa         := 0;
     if vEfectivo > 0 then
       AR^.Debito := vEfectivo + vAportesAAbonar + vDescuentoAAbonar
     else
     if vNacional > 0 then
       AR^.Debito := vNacional + vAportesAAbonar + vDescuentoAAbonar
     else if vLibranza > 0 then
       AR^.Debito := vLibranza + vAportesAAbonar + vDescuentoAAbonar
     else AR^.Debito := vTotalLiquidacion + vAportesAAbonar + vDescuentoAAbonar;
     AR^.Credito      := 0;
     AR^.EsCapital := False;
     AR^.EsCausado := False;
...
// Es pago en Efectivo
      if vEfectivo > 0 then
      begin
        if vTotalLiquidacion < vEfectivo then
         begin
           with IBQOtros do
           begin
           SQL.Clear;
           ...
           ParamByName('ID_CLASIFICACION').AsInteger := Linea; //SE CAMBIO Clasificacion;
           ...
           vMenorValor := vEfectivo - vTotalLiquidacion;
           ...
           AR^.Tasa         := 0;
           AR^.Debito       := 0;
           AR^.Credito      := vEfectivo - vTotalLiquidacion;
         ...
         if (vTotalLiquidacion > vEfectivo) then
          begin
           ...
           SQL.Add('ID_GARANTIA = :"ID_GARANTIA" and ');
           SQL.Add('ID_CATEGORIA = :"ID_CATEGORIA" ');
           ParamByName('ID_CLASIFICACION').AsInteger := Linea; //Clasificacion;
           ParamByName('ID_GARANTIA').AsInteger      := Garantia;
           ParamByName('ID_CATEGORIA').AsString     := Categoria;
           IBQOtros.Open;
           if RecordCount = 0 then
            ...
            vMenorValor := vTotalLiquidacion - vEfectivo;
            CodigoAhorros := IBQOtros.FieldByName('COD_CAPITAL_CP').AsString;
            New(AR);
            AR^.CuotaNumero := CuotaNumero;
            AR^.CodigoPuc   := CodigoAhorros;
            ...
            AR^.Dias         := 0;
            AR^.Tasa         := 0;
            AR^.Debito       := (vTotalLiquidacion - vEfectivo);
            AR^.Credito      := 0;
...
// Es pago por Bancos
      ...
      begin
        ...
        if vTotalLiquidacion < (vNacional - vComisionNal) then
         ...
         if (vTotalLiquidacion > (vNacional - vComisionNal)) then
          begin
             MessageDlg('No se puede liquidar esta Colocación, Valor Consignado no es suficiente',mtError,[mbOk],0);
             Close;
             Exit;
          end;
          {
      ...
      begin
        if vTotalLiquidacion < vLibranza then
         begin
           with IBQOtros do
           begin
           SQL.Clear;
           ...
           AR^.Tasa         := 0;
           AR^.Debito       := 0;
           AR^.Credito      := (vLibranza - vTotalLiquidacion);
           CuotasLiq.NuevoSaldo := CuotasLiq.NuevoSaldo - AR^.Credito;
           AR^.EsCapital := True;
           AR^.EsCausado := False;
           AR^.EsCorriente := False;
        ...
        { if (vTotalLiquidacion > vLibranza) and (vTotalAhorros >= (vTotalLiquidacion - vLibranza)) then
          begin
            New(AR);
            AR^.CuotaNumero := CuotaNumero;
            AR^.CodigoPuc   := CodigoAhorros;
            ...
            AR^.Dias         := 0;
            AR^.Tasa         := 0;
            AR^.Debito       := (vTotalLiquidacion - vLibranza);
            AR^.Credito      := 0;
            AR^.EsCapital := False;
            AR^.EsCausado := False;
            AR^.EsCorriente := False;
            ...
            AR^.EsOtros := False;
            if (AR^.Debito <> 0) or (AR^.Credito <> 0) then CuotasLiq.Lista.Add(AR);
            RetiroExtracto := (vTotalLiquidacion - vLibranza);
            BaseGMF := BaseGMF + RetiroExtracto;
          end; // Fin vNacional
          }
     end;

     if vAportesAAbonar > 0 then begin
           New(AR);
           AR^.CuotaNumero := 0;
     ...
     if vDescuentoAAbonar > 0 then begin
           New(AR);
           AR^.CuotaNumero := 0;
           AR^.CodigoPuc   := CodigoDescuento;
           AR^.FechaInicial := fFechaActual;
           AR^.FechaFinal   := fFechaActual;
           AR^.Dias         := 0;
           AR^.Tasa         := 0;
           AR^.Debito       := 0;
           AR^.Credito      := vDescuentoAAbonar;
           //CuotasLiq.NuevoSaldo := CuotasLiq.NuevoSaldo - AR^.Credito;
           AR^.EsCapital := False;
           AR^.EsCausado := False;
           AR^.EsCorriente := False;
...

{
     if (not CredFallecido) and (not CredIncapacitado) then begin
      if vTotalLiquidacion > (vTotalAhorros + vLibranza + (vNacional - vComisionNal)) then
       begin
...
//     frmMuestroLiquidacionColocacion.FechaCorte := EdFechaCorte.Date;
     ...
     frmMuestroLiquidacionColocacion.AbonoAportes := vAportesAAbonar;
     frmMuestroLiquidacionColocacion.AbonoDescuento := vDescuentoAAbonar;
