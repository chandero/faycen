...
type
  Tfrmcierredia = class(TForm)
    ...
    IBQRepanormalABREVIATURA: TIBStringField;
...
begin
   ...
   with querycompcierre do
    begin
      ...
      sql.Clear;
      sql.Add('select ');
      sql.Add('c.TIPO_COMPROBANTE,');
      sql.Add('t.ABREVIATURA,');
      sql.Add('t.DESCRIPCION,');
      sql.Add('c.ID_COMPROBANTE,');
      sql.Add('c.ID_AGENCIA,');
      sql.Add('c.FECHADIA,');
      sql.Add('c.TOTAL_DEBITO,');
      sql.Add('c.TOTAL_CREDITO,');
      sql.Add('c.ESTADO');
      sql.Add('from "con$comprobante" c');
      sql.Add('inner join "con$tipocomprobante" t on t.ID = c.TIPO_COMPROBANTE');
      sql.Add('where c."FECHADIA" BETWEEN :FECHAINICIAL AND :FECHAFINAL and c.ESTADO = :ESTADO');
      sql.Add('order by c.TIPO_COMPROBANTE ASC, c.ID_COMPROBANTE ASC');
      ParamByName('FECHAINICIAL').AsDate := edfechainicial.Date;
      ParamByName('FECHAFINAL').AsDate := edfechafinal.Date;
      ...
      if querycompcierre.RecordCount = 0 then
       begin
         MessageDlg('No exiten comprobantes de fecha' + #10 + #13 + datetostr(edfechainicial.Date) + ' a ' + datetostr(edfechafinal.Date) + #13 + #10 + 'Para Cerrar',mterror,[mbok],0);
         exit;
       end;
       ...
       frmProgreso.Ejecutar;
        While not QueryCompcierre.Eof do
         begin
           ...
           abreviatura := FieldByName('ABREVIATURA').AsString;
           frmProgreso.InfoLabel := 'Comprobante: '+Format('%s %.7d',[descripcion,id_comprobante]);
              ...
              if total_debito <> total_credito then
                 with IBSQLcierre do
                   begin
                     ...
                     sql.Add('"con$errorcierrecomp"."ABREVIATURA")');
                     sql.Add(' values(');
                     sql.Add(':"ID_AGENCIA",:"ID_COMPROBANTE",:"ERROR",:"TIPO_COMPROBANTE",:"ABREVIATURA")');
                     ...
                     ParamByName('ABREVIATURA').AsString := abreviatura;
                     ExecQuery;
                     vcuadrado := false;
                     close;
                   end;
...
procedure Tfrmcierredia.edfechainicialKeyPress(Sender: TObject;
  var Key: Char);
begin
        EnterTabs(Key,Self);
end;
