...
type
  TFrmDesembolso = class(TForm)
    ...
    Label15: TLabel;
    IBQtmp: TIBQuery;
    CDcolParciallinea: TIntegerField;
    CDcuotaslinea: TIntegerField;
    ...
    dsFormaDesembolso: TDataSource;
    IBQformadesembolso: TIBQuery;
    btnBuscarPersona: TButton;
    dbProveedor: TDBLookupComboBox;
    panelFormaDesembolso: TPanel;
    Label22: TLabel;
    dbFormaDesembolso: TDBLookupComboBox;
...
var
  ...
  vDescuentoLiquidacionTotal: Currency;
...
begin
        ...
        IBQformadesembolso.Database := dmGeneral.IBDatabase1;
        IBQformadesembolso.Transaction := dmGeneral.IBTransaction1;
...
//            EdCuentaAportes.Caption := IntToStr(vCtaAportes);
            ...
            if (vLinea = 2) then
            begin
                cbFormaDesembolso.ItemIndex := 4;
                panelBancoDesembolso.Visible := False;
                panelFormaDesembolso.Visible := True;
                panelProveedor.Visible := False;
                IBQformadesembolso.First;
                dbFormaDesembolso.KeyValue := 1;
            end;
            if (vLinea = 3) then
            begin
                cbFormaDesembolso.ItemIndex := 3;
                panelBancoDesembolso.Visible := False;
                panelFormaDesembolso.Visible := False;
...
//                end;
//            end;
///// Validacion vencimiento terminos
            // IDENTIFICA LA LINEA
            Close;
            SQL.Clear;
...
//        BTfng.Enabled := True;
        ...
        IBQproveedor.Last;
        IBQproveedor.First;
        IBQformadesembolso.Close;
        IBQformadesembolso.Open;
        IBQformadesembolso.Last;
        IBQformadesembolso.First;
end;
...
begin
        ...
        if (cbFormaDesembolso.ItemIndex = 1) or (cbFormaDesembolso.ItemIndex = 2) then
        begin
            if (lstBancos.KeyValue < 1) then
            begin
        ...
        if (cbFormaDesembolso.ItemIndex = 3) then
        begin
             if (dbProveedor.KeyValue < 1) then
             begin
                 ShowMessage('Debe Seleccionar el Proveedor');
                 dbProveedor.SetFocus;
                 Exit;
             end;
        end;
        
        if (cbFormaDesembolso.ItemIndex = 4) then
        begin
              if (dbFormaDesembolso.KeyValue < 1) then
              begin
                  ShowMessage('Debe Seleccionar la Forma de Desembolso');
                  dbFormaDesembolso.SetFocus;
                  Exit;
              end;
        end;
   ...
   begin
     ...
     begin
       ...
       begin
          ...
          FieldValues['linea'] := IBSQL2.FieldByName('ID_LINEA').AsInteger;
...
var CuotasLiq:TCuotasLiq;
    ...
    CapitalNomina:Currency;
    InteresNomina:Currency;
    CapitalCuota:Currency;
...
begin
      ...
      //  codigoahorros := recupera_codigo(8);
        ...
        vDescuentoLiquidacionTotal := 0;
        fechahoy := vFechaDesembolso;
        CuotasLiq.Lista := TList.Create;
        with CDcolParcial do
        begin
          First;
          while not Eof do
          begin
            ...
            //Leer Nomina No Aplicada

            IBQtmp.Database := dmGeneral.IBDatabase1;
            IBQtmp.Transaction := dmGeneral.IBTransaction1;
            {
            IBQtmp.SQL.Clear;
            IBQtmp.SQL.Add('SELECT * FROM GEN$DESCUENTOCARTERA l WHERE l.ID_COLOCACION = :ID_COLOCACION and l.APLICADO = 0');
            IBQtmp.ParamByName('ID_COLOCACION').AsString := id_colocacion;
            IBQtmp.Open;
            while not IBQtmp.Eof do
            begin
              CapitalNomina := IBQtmp.FieldByName('CAPITAL').AsCurrency;
              SaldoActual := SaldoActual - CapitalNomina;
              FechaPagoI := CalculoFecha(FechaPagoI, AmortizaI);
              IBQtmp.Next;
            end;
            }
            vSaldoActual := SaldoActual;
            //
              DiasHastaElFin := DiasEntre(FechaPagoI,Now()); 
            //Nueva Liquidacion Total
            //  InteresCuota := SimpleRoundTo((SaldoActual * (TasaNominalVencida(ValorTasa,AmortizaI) + PuntosAdic) / 100 * DiasHastaElFin ) / 360,0);
              InteresCuota := 0;
              ...
              begin
                ...
                SQL.Add('ID_CATEGORIA = :"ID_CATEGORIA" and ');
                SQL.Add('ID_LINEA = :"ID_LINEA" ');
              ...
              AR^.CodigoPuc := IBQtmp.FieldByName('COD_CAPITAL_CP').AsString;
              AR^.FechaInicial := FechaCorte;
              AR^.FechaFinal   := FechaCorte;
              AR^.Dias         := 0;
              AR^.Tasa         := 0;
              AR^.Debito       := 0;
              AR^.Credito      :=  CapitalCuota;
              vDescuentoLiquidacionTotal := vDescuentoLiquidacionTotal + CapitalCuota;
              AR^.EsCapital := True;
              AR^.EsCausado := False;
              ...
              AR^.CodigoPuc := IBQtmp.FieldByName('COD_INT_MES').AsString;
              AR^.FechaInicial := FechaCorte;
              AR^.FechaFinal   := FechaCorte;
              AR^.Dias         := 0;
              AR^.Tasa         := 0;
              AR^.Debito       := 0;
              AR^.Credito      :=  InteresCuota;
              AR^.EsCapital := False;
              AR^.EsCausado := False;
              AR^.EsCorriente := True;
              AR^.EsVencido := False;
        ...
        begin
          ...
          begin
            ...
            if (TipoCuota = 1) then
               ...
               CuotasLiq,Clasificacion,Garantia,Categoria,Linea,Estado,ValorCuota,
               FechaPagoK,FechaPagoI,TipoInteres,ValorTasa,ValorMora,PuntosAdic,SaldoActual,AmortizaK,AmortizaI,0{DiasProrroga})
            else if (TipoCuota = 2) then
...
begin
  ...
  try
        ...
        vCaja := vDesembolso - vDescuentoLiquidacionTotal;
        vFechaHora := Date;
        TipoCaptacion := 1;
        with IBQuery do
        begin
...
{Contabilizo Corto y Largo Plazo}
           with dmColocacion.IBDScontable do
            begin
              Open;
              if (vCortoPlazo+vLargoPlazo) > 0 then
              begin
                 Append;
                 ...
                 FieldByName('DEBITO').AsCurrency := vCortoPlazo+vLargoPlazo;
            ...
            if vDescuento > 0 then begin
              ...
              // extracto interes por fecha de pago
                  ...
                  begin
                    ...
                    // Proveedor
                       3: begin
                            vCodigoPuc := IBQproveedor.FieldByName('CODIGO').AsString;
                          end;
                    // Emergencia  u Otros
                       4: begin
                            vCodigoPuc := IBQformadesembolso.FieldByName('CODIGO').AsString;
                          end;
                    end;
...
//credito no visado por defecto
                ...
                begin
                  SQL.Clear;
                  SQL.Add('insert into GEN$COLOCACIONPROVEEDOR VALUES (');
                  SQL.Add(':ID_COLOCACION, :ID_PROVEEDOR)');
                  ParamByName('ID_COLOCACION').AsString := VidColocacion;
                  ParamByName('ID_PROVEEDOR').AsInteger := dbProveedor.KeyValue;
                  ExecSQL;
                  Close;
                end;
...
procedure TFrmDesembolso.btnBuscarPersonaClick(Sender: TObject);
...
begin
                ...
                begin
                   ...
                   vTipoId := frmBuscarPersona.id_identificacion;
                   EdNumeroIdentificacion.Text := frmBuscarPersona.id_persona;

                   EdNumeroIdentificacion.OnExit(self);
                end;
