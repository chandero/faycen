...
uses digitacuenta, UnitCreaciondePersona, Unit_buscarcomprobante,unit_Dmpuc, UnitGlobales, Unit_Mantenimientopuc;
...
begin
                if (estadoc= 'N') and (estadoa= 'N')  then
                 begin
                  buscarcomprobante(id_tipocomprobante, id_comprobante,id_agencia);
                  bloquear;
...
// Fin Obtener Consecutivo
         ...
         begin
            ...
            ibsql.SQL.Add(':PLAN_ID,:PLDE_ORDEN,:PLDE_CODIGO,:PLDE_DEBITO,:PLDE_CREDITO)');
            ARecord := List.Items[I];
            ibsql.ParamByName('PLAN_ID').AsInteger := plantillaID;
...
begin
              ...
              IBPagar.ParamByName('FECHA_MOVIMIENTO').AsDateTime := EdFecha.DateTime;  //StrToDate(FechaDia.Caption);
              IBPagar.ParamByName('HORA_MOVIMIENTO').AsTime := Time;
              if ValorD > 0 then Valor := ValorD else Valor := ValorC;
              if Valor > 0 then
              begin
               if dblcbtipo.KeyValue = 1 then
