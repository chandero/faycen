...
type
  TfrmProcesarDatacredito = class(TForm)
    ...
    CheckBox6: TCheckBox;
    IBSQL1: TIBSQL;
    DataActualDIASMORA: TStringField;
    DataActualFECHAEXIG: TStringField;
    DataActualCODMUNICIPIO: TStringField;
...
//             DiasMora := ObtenerDeudaFecha(IBQuery1.FieldByName('ID_AGENCIA').AsInteger,IBQuery1.FieldByName('ID_COLOCACION').AsString,EdFechaCorte.Date,IBSCodeudores).Dias;
             ...
             if (IBQuery1.FieldByName('ID_ESTADO').AsInteger = 2) then
               begin
                 IBSQL1.Close;
                 IBSQL1.SQL.Clear;
                 IBSQL1.SQL.Add('SELECT FECHA_CAMBIO FROM "col$cambioestado" WHERE ID_ESTADO_ACTUAL = 0 AND ID_ESTADO_NUEVO = 2 AND ID_AGENCIA = :AGENCIA AND ID_COLOCACION = :COLOCACION');
                 IBSQL1.ParamByName('AGENCIA').AsInteger := IBQuery1.fieldbyname('ID_AGENCIA').AsInteger;
                 IBSQL1.ParamByName('COLOCACION').AsString := IBQuery1.FieldByName('ID_COLOCACION').AsString;
                 IBSQL1.ExecQuery;
                 if IBSQL1.RecordCount > 0 then
                   LineaC.FechaExig := DateToStr(IBSQL1.FieldByName('FECHA_CAMBIO').AsDate)
                 else
                   LineaC.FechaExig := '';
                 IBSQL1.Close;
               end
            ...
            LineaC.CodMunicipio := IntToStr(IBSClientes.FieldByName('COD_MUNICIPIO').AsInteger);
            ...
            DataActual.FieldByName('DIASMORA').AsString := LineaC.DiasMora;
            DataActual.FieldByName('FECHAEXIG').AsString := LineaC.FechaExig;
            DataActual.FieldByName('CODMUNICIPIO').AsString := LineaC.CodMunicipio;
            DataActual.Post;
            DataActual.Close;
            ...
            //   ColocacionNew := Format('%.16d',[ConverColocacion]);
             ...
             //  NovedadLocal := 0;
              ...
              with IBSCuotas do begin
               ...
               ExecQuery;
               LineaC.CodMunicipio := IntToStr(IBSCuotas.FieldByName('MUNICIPIO').AsInteger);
